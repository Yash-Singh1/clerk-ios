...
struct ObservableObjectBackportTests {
  ...
  @Test
  func nestedPagerMutationPublishes() {
    let dataSource = OrganizationAccountListDataSource()
    var didChange = false
    let observation = dataSource.objectWillChange.sink {
      didChange = true
    }

    dataSource.membershipsPager.isLoadingMore = true
